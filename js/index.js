...
var ztsyswiper = new Swiper('#indexsydivcon',{ pagination:'#indexsydivcon .paginationhd', paginationClickable:true,observer:true,observeParents:true });
	...
	$.ajax({
		...
		success: function(data){
			if(data.error == 200){
				...
				var img_html = '<img src="'+data.images+'" />';
				$(".pro_img").html(img_html);			
...
switch(xmPid)
{
		...
		success: function(data){
			if(data.error == 200){
				...
				for(var i=0; i<data.length; i++){
					...
					html += '<em data-banid="'+data[i].ban+'" building_name="'+data[i].building+'" class="';
				...
				for(var k=0; k<data3.length; k++){
					ban_array[k] = parseInt(data3[k].ban);
				}

				ban_array.sort(function (x,y) {
			      return x-y;
			    });


				for(var j=0; j<ban_array.length; j++){
					$(".indexcon h5 em:eq("+ ban_array[j] +")").attr("data-banid",ban_array[j]);
				}*/
			    ...
			    var louz_status = getQueryString("lou_status");
			    var louzz_status;
			    if(louz_status == "no_sell_status"){
			    	louzz_status = "no_sell";
			    }else if(louz_status == "has_sell_status"){
			    	louzz_status = "has_sell";
			    }else if(louz_status == "no_kp_status"){
			    	louzz_status = "no_kp";
			    }
	...
	$(document).on("click",".indexcon h5 em",function(){
		...
		var sell_status = $(this).attr("sell_status");
		
		//$("body").css({"height":""+bodyheight+"px","overflow":"hidden"});
		$(".indexsingle .indexsinglediv").attr("data-banid",""+banid+"");
		$(".indexsingle .indexsinglediv").attr("xs_status",""+sell_status+"");
		$(".indexsingle .indexsinglediv").css({"marginTop":""+listdivtop+"px","height":""+listdivheight+"px"});
		$(".indexsingle .indexsinglediv .indexsinglecon").css("height",""+listdivheight+"px");
		$(".indexsingle .indexsinglediv .indexsinglecon .indexsinglelist").css("height",""+listdivconheight+"px");
		$(".indexsingle .indexsinglediv h6").css("marginTop",""+listh6top+"px");
	...
	$(document).on("click",".indexsingle .indexsinglediv .indexsinglecon .indexsingletop h4 i",function(){
		...
		var xs_status = $(".indexsingle .indexsinglediv").attr("xs_status");
		var unit = $(this).attr("data-unit");
		$(this).addClass("hover").siblings().removeClass("hover");
		listcon(banid,unit,xs_status);
	});
	...
	function listcon(banid,unit,xs_lou_status){
		var paging = 0;
		if(unit > 0){
			paging = 1;
			unit = "&unit="+unit+"";
				...
				if(data.error == 200){
					...
					if(data.data.length > 0){
						...
						for(var u=0; u<data.data.length; u++){
							html += "<div class='clearfix' style='width:100%;height:63px;margin:0 0 25px 0'>"
							for(var i=0; i<data.data[u].length; i++){
								if(data.data[u][i].unit > 0){
									html += '<li data-id="'+data.data[u][i].id+'" ';

									if(xs_lou_status == "no_sell"){
										html += 'id="no_sell_status"'
									...
									if(xs_lou_status == "has_sell"){
										html += 'id="has_sell_status"';
										html += ' class="statusa"><b class="itema">';
									}

									if(xs_lou_status == "no_kp"){
										html += 'id="no_kp_status"';
										html += ' class="statusb"><b class="itemc">';
									}

									
									html += '<span class="danyuan">'+data.data[u][i].unit+'单元</span><span class="danshi">'+data.data[u][i].room+'室</span></b><p>*面积：'+data.data[u][i].area+' m²</p><p>*房型：'+data.data[u][i].type+'</p></li>';
	...
	$(document).on("click",".indexsingle .indexsinglediv .indexsinglecon .indexsinglelist li",function(){
		...
		location.href = "prod.html?&id="+ id + htmlVtimeb +"&signStatus="+liStatus+"&idStatus="+li_id;
	});
			...
			if(data.error == 200){
				...
				if(data.length > 0){
					for(var u=0; u<data.length; u++){
						html += '<li><div>';
						html += '<p class="huhao"><b>签约户号:</b><em>'+ data[u].ban +'幢'+ data[u].unit +'单元'+ data[u].room +'室</em></p>';
...
function controljava(eq){
	...
	var em_length = $(".indexcondiv .indexcon h5 em").length;
	var em_array = []
	for(var k=0; k<em_length; k++){
		if(parseInt($(".indexcondiv .indexcon h5 em:eq("+k+")").attr("data-banid")) != 0){
			var ban_id = parseInt($(".indexcondiv .indexcon h5 em:eq("+k+")").attr("data-banid"));
			break;
		}
	}
	

	indexxkajax(1,ban_id);
	...
	$.ajax({
		...
		success: function(data){
			if(data.error == 200){
				...
				var ban_Arr = [];
				for(var m=0; m<datal.unit.length; m++){
					ban_Arr[m] = parseInt(datal.unit[m].ban);
					
				}

				ban_Arr.sort(function (x,y) {
			      return x-y;
			    });
				
				for(var p=1; p<ban_Arr.length+1; p++){
					if(p == page){
						htmlpage += '<i class="hover" data-ban="'+ ban_Arr[p-1] +'">'+p+'</i>';
					}else{
						htmlpage += '<i data-ban="'+ ban_Arr[p-1] +'">'+p+'</i>';
					}
				}
	...
	$.ajax({
		...
		success: function(data){
			if(data.error == 200){
				...
				htmlnav += '<li style="display:none"><b data-index="0">基础信息</b></li>';
				...
				var html_slide = '';
				for(var i=0; i<ztsyimgurl[1].length; i++){

					if(sb_data == 'pc'){
						html_slide += '<div class="swiper-slide"><img src="'+ ztsyimgurl[1][i] +'"  /></div>';
					}else{
						html_slide += '<div class="swiper-slide"><a href="'+ztsyimgurl[1][i]+'"><img src="'+ ztsyimgurl[1][i] +'"  /></a></div>';
					}
		
				}
				$(".businesscon .businessdiv .businesslist .indexsydiv .indexsydivcon .swiper-wrapper").html(html_slide);

				if(ztsyimgurl[1].length > 1){
				...
				$(".businesscon .businessdiv .businessnav ul li:eq(1)").addClass("hover");
	/*$(".businesscon .businessdiv .businesslist ul").show();*/
	/*$(".businesscon .businessdiv .businesslist .indexsydiv").hide();*/
	/*$(".businesscon .businessdiv .businesslist ul li").css({"height":""+businesslistheight+"px"});*/	
...
for(var i=0; i<ztsyimgurlcon[k].length; i++){
	html_ipad += '<div class="swiper-slide"><a href="'+ztsyimgurlcon[k][i]+'"><img src="'+ ztsyimgurlcon[k][i] +'"  /></a></div>';
}

if(datas == 'pc'){
	$(".businesscon .businessdiv .businesslist .indexsydiv .indexsydivcon .swiper-wrapper").html(html);
}else{
	$(".businesscon .businessdiv .businesslist .indexsydiv .indexsydivcon .swiper-wrapper").html(html_ipad);
}
