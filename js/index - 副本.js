...
switch(xmPid)
{
	...
	$.ajax({
		type:"get",
		url:""+ApiUrl+"Home/Index/main.html?pid=4&v=1512055471049",
		data:"",
		dataType:"json",
		...
		success: function(data){
			...
			if(data.error == 200){
				...
				for(var i=0; i<data.length; i++){
					...
					html += '<em building_name="'+data[i].building+'" data-banid="'+data[i].id+'" class="';
					...
					html += '" style="left:'+data[i].xz+'px;top:'+data[i].yz+'px;">'+data[i].building+'#</em>';
				}
	...
	function listcon(banid,unit){
		...
		$.ajax({
			type:"get",
			url:""+ApiUrl+"Home/Index/unit.html?&ban="+ banid + "&pid=4" + unit + ajaxtimea +"",
			data:"",
			dataType:"json",
			async: false,
			//beforeSend: function(request){ ajaxtoken(request); },
	...
	$(document).on("click",".indexsingle .indexsinglediv .indexsinglecon .indexsinglelist li.statusb",function(){
		...
		$.ajax({
			...
			success: function(data){
				if(data.error == 200){
					...
					if(data.length > 0){
						for(var i=0; i<data.length; i++){
							html += '<p><b>'+data[i].title+'</b><em>打印</em><i>';
							for(var g=0; g<data[i].img.length; g++){
								html += '<img src="'+data[i].img[g]+'" />';
							}
							html += '</i></p>';
						}
...
function timegs(sj,shape){
	...
	if(shape == 1){
		var timesj = year +"年"+ month +"月"+ dater ;
	}else if(shape == 2){
